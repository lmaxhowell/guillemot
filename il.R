...
#   skip <- which(row.names(psi)=="S")
#   f <- function(ii){
#     # print(c(ii,a+j-1,dim(phi),phi[ii,a+j-1,2]))
#     return(phi[ii,a+j-1,2]*psi[2,2,ii,a+j-1])
#   }
#   g <- function(){
#     return(phi[t,a,1]*psi[1,2,t,a])
#   }
#   h <- function(ii){
#     # print(c(ii,a+j-1,a,j,t))
#     return(1-phi[ii,a+ii-t,2])
#   }
#   if(r %in% 3:7){
#     prob <- phi[t,a,r]*psi[r,skip,t,a]*(1-phi[t,a,skip]) + (1-phi[t,a,r])
#   }else if(r==1){
...
#       for(i in (t+2):(Time)){
#         print(c((t+2),(Time)))
#         j <- i-(t+2)+1 # this is the place along the loop that we are
#         # print(sapply((t+2):i,function(x) f(x)))
#         # print((t+2):i)
#         # print(c(t,i,Time))
#         if((t+2)>i){
...
#         }
#         p2 <- p2 + g()*prod(sapply((t+2):i,function(x) f(x)))*h(i)
#       }
#     }
# 
#     # prob_c <- rep(0,Time-t)
#     # prob_c[1] <- phi[t,a,r]*psi[r,2,t,a]
#     # # print(prob_c)
#     # if((Time-t)>1){
#     #   for(i in (t+2):Time){
#     #     # print(c(i,t,i-t-1,i-t))
#     #     prob_c[i-t] <- prob_c[i-t-1]*phi[i,a+i-t,2]*psi[2,2,i,a+i-t]
#     #   }
#     #   for(i in t:(Time-1)){
#     #     # print(c(a,i,t,a+i-t))
#     #     prob_c[i-t] <- prob_c[i-t]*(1-phi[i,a+i-t,2])
#     #   }
#     # }else if((Time-t)==1){
#     #   prob_c[1] <- phi[t,a,r]*psi[r,2,t,a]
#     # }
#     # prob <- (1-phi[t,a,r]) + sum(prob_c)
#     print(c(p1,p2))
#     prob <- p1 + p2
#   }
#   if(!exists("prob")){
...
#   Ages <- dim(delta)[2]
#   # print(sapply(list(delta,kap,rho,gam,epsilon),ncol))
#   if(ncol(kap)!=Time | ncol(rho)!=Time | ncol(gam)!=Time | ncol(epsilon)!=Time | ncol(iota)!=Time){
#     stop("delta, kappa, rho, gamma, epsilon and iota must all have the same number of columns, corresponding to time")
#   }
#   psi <- array(0,dim=c(length(states),length(states),Time,Ages))
#   for(t in 1:Time){
#     for(a in 1:Ages){
#       psi[1,,t,a] <- c((1-epsilon[t,a,1])*(1-rho[t,a,1]),epsilon[t,a,2],(1-epsilon[t,a,3])*rho[t,a,3],0,0,0,0,0)
#       psi[2,,t,a] <- c(iota[t,a,1]*(1-rho[t,a,1]),1-iota[t,a,2],iota[t,a,3]*rho[t,a,3],0,0,0,0,0)
#       row.names(psi) <- states
#       colnames(psi) <- states
#       # this is if these rows depend on the ROWS theyre in
#       # for(i in 4:8){
...
# }
# 
# ll.il2 <- function(theta,phi.ind,delt.ind,kap.ind,rho.ind,gam.ind,eps.ind,iot.ind,struc,ch){
#   states <- c("N","E","B1","LB","L_B","LB_","L_B_","S")
#   
#   phi <- untrans(logistic(theta[phi.ind]),struc$phi$age,struc$phi$time,struc$phi$state)
#   delt <- untrans(logistic(theta[delt.ind]),struc$delt$age,struc$delt$time,struc$delt$state)
...
#   gam <- untrans(logistic(theta[gam.ind]),struc$gam$age,struc$gam$time,struc$gam$state)
#   eps <- untrans(logistic(theta[eps.ind]),struc$eps$age,struc$eps$time,struc$eps$state)
#   iot <- untrans(logistic(theta[iot.ind]),struc$iot$age,struc$iot$time,struc$iot$state)
#   
#   psi <- make.psi2(delt,kap,rho,gam,eps,iot)
#   # print(psi[,,1,1])
#   # print(logistic(theta[eps.ind]),logistic(theta[iot.ind]))
#   
#   ll <- il2(ch,phi,psi)
#   
#   return(ll)
