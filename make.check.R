...
if(l==1){
  ...
  df.mle <- data.frame("par"=rep(par.name,n),
                       ...
                       "rep"=rep(1:n,each=12),
                       "start"=c(sapply(1:n,function(x) op[[x]]$start )))
  df.mle2 <- data.frame("par"=rep(par.name,n),
                        "mle"=c(sapply(1:n,function(x) par.fun(op2[[x]]$estimate))),
                        "convergence"=rep(sapply(1:n,function(x) op2[[x]]$code),each=12),
                        "rep"=rep(1:n,each=12),
                        "start"=c(sapply(1:n,function(x) op2[[x]]$start )))
  ...
  df.mle.phi2 <- data.frame("phi"=c(sapply(1:n,function(x) c(logistic(phi.trans(op2[[x]]$estimate[1:3],op2[[x]]$estimate[4:5],beta.struc)[,1:4,1])))),
                            "ageclass"=as.factor(rep(rep(1:4,each=15),n)),
                            "time"=as.factor(rep(1:15,4*n)),
                            "convergence"=rep(sapply(1:n,function(x) op2[[x]]$code),each=4*15),
                            "rep"=rep(1:n,each=4*15))
  ...
  save(df.mle,df.true,df.mle2,df.mle.phi,df.mle.phi2,file="code.check.RData")
}