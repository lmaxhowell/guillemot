...
if(l==1){
  ...
  TimeClasses <- 2:15
  cores <- length(TimeClasses)*6
  tcr <- rep(TimeClasses,6)
  states <- c("N","E","B1","LB","L_B","LB_","L_B_","S")
  Time <- ncol(ch)-1
  op <- mclapply(tcr, function(x){
          timeclasses <- 2
          ageclasses <- 4
  ...
  save(op,file="MS2op.RData")
  
  sel <- list()
  for(i in 1:length(TimeClasses)){
    convergences <- sapply(seq(i,84,14),function(x) op[[x]]$code)
    mins <- sapply(seq(i,84,14),function(x) op[[x]]$minimum)
    converged <- which(convergences %in% 1:2)
    if(length(converged)!=0){
      selected <- converged[which.min(mins[converged])]
    }else if(length(converged)==0){
      selected <- which.min(mins)
    }
    sel[[i]] <- op[[seq(i,84,14)[selected]]]
  }
  ...
  aics1.all <- sapply(1:length(tcr),function(x) AICc(op[[x]]$minimum,n1))
  aics2.all <- sapply(1:length(tcr),function(x) AICc(op[[x]]$minimum,n2))
  aics1 <- sapply(1:length(TimeClasses),function(x) AICc(sel[[x]]$minimum,n1))
  aics2 <- sapply(1:length(TimeClasses),function(x) AICc(sel[[x]]$minimum,n2))
  
  df.aic1 <- data.frame("AICc"=aics1,
                        "TimeChange"=TimeClasses,
                        "convergence"=sapply(TimeClasses,function(x) sel[[x-1]]$code))
  df.aic2 <- data.frame("AICc"=aics2,
                        "TimeChange"=TimeClasses,
                        "convergence"=sapply(TimeClasses,function(x) sel[[x-1]]$code))
  df.aic1.all <- data.frame("AICc"=aics1.all,
                            "TimeChange"=tcr,
                            "convergence"=sapply(1:length(tcr),function(x) op[[x]]$code))
  df.aic2.all <- data.frame("AICc"=aics2.all,
                            "TimeChange"=tcr,
                            "convergence"=sapply(1:length(tcr),function(x) op[[x]]$code))
  ...
  save(df.aic1,df.aic2,df.aic1.all,df.aic2.all,file="MS.time2.RData")
  print("saved 1")
  ...
  df.par <- data.frame("par"=rep(nam,length(TimeClasses)),
                       "mle"=c(unlist(sapply(TimeClasses,function(x) par.fun(sel[[x-1]]$estimate)))),
                       "TimeChange"=rep(TimeClasses,each=length(theta.s(4))),
                       "convergence"=rep(sapply(TimeClasses,function(x) sel[[x-1]]$code),each=length(theta.s(4))),
                       "AICc"=rep(aics1,each=length(theta.s(4))),
                       "AICc_un"=rep(aics2,each=length(theta.s(4))),
                       "start"=c(unlist(sapply(TimeClasses,function(x) sel[[x-1]]$start))))
  
  df.par.all <- data.frame("par"=rep(nam,length(TimeClasses)*6),
                           "mle"=c(unlist(sapply(1:length(tcr),function(x) par.fun(op[[x]]$estimate)))),
                           "TimeChange"=rep(rep(TimeClasses,each=length(theta.s(4))),6),
                           "convergence"=rep(sapply(1:length(tcr),function(x) op[[x]]$code),each=length(theta.s(4))),
                           "AICc"=rep(aics1.all,each=length(theta.s(4))),
                           "AICc_un"=rep(aics2.all,each=length(theta.s(4))),
                           "start"=c(unlist(sapply(1:length(tcr),function(x) op[[x]]$start))))
  
  save(df.aic1,df.aic2,df.aic1.all,df.aic2.all,df.par,file="MS.time2.RData")
  print("saved 2")
}