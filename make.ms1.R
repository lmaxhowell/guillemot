...
ll.il.ms <- function(theta,ageclasses,ch){
  ...
  beta <- theta[ageclasses:(Time+ageclasses-1)]
  rho.input <- theta[(Time+ageclasses):(Time+ageclasses+2)]
  ...
  theta <- theta[-c(1:(Time+ageclasses+2))]
...
ll.il.ms.s <- function(theta,ageclasses,ch){
  ...
  kap <- untrans(logistic(c(0,theta[(Time+ageclasses+2+5):(Time+ageclasses+2+5+4)])),struc.kap$age,struc.kap$time,struc.kap$state)
    
  theta <- theta[-c(1:(Time+ageclasses+2+5+4))]
...
theta.s <- function(n,state.depen=FALSE){ # want n to be desired number of age classes
  x <- ifelse(state.depen==FALSE,22,31)
...
op <- mclapply(AgeClasses,function(x) optim(theta.s(x,FALSE),ll.il.ms,ageclasses=x,ch=ch,
                                     control=list(fnscale=-1,maxit=10000),method="Nelder-Mead",hessian=TRUE),mc.cores=cores)
...
save(df.aic,op,op.s,file="ModelSelection.RData")
...
save(df.par,df.aic,op,op.s,file="ModelSelection.RData")